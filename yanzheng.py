...
def select_source_path(infer_type):
    """根据推理类型弹出窗口，手动选择路径 infer_type: single_img/batch_img/video.
    """
    # 隐藏tkinter主窗口
    root = tk.Tk()
    root.withdraw()
    ...
    if infer_type == "single_img":
        ...
        # 选择文件夹
        file_path = filedialog.askdirectory(title="选择要批量检测的图片文件夹")
    elif infer_type == "video":
        # 选择视频文件（或输入摄像头ID）
        choice = input("请选择：1-检测视频文件  2-检测摄像头（输入数字）：")
        if choice == "1":
            file_path = filedialog.askopenfilename(
                title="选择要检测的视频文件", filetypes=[("视频文件", "*.mp4 *.avi *.mov *.mkv"), ("所有文件", "*.*")]
            )
        else:
...
def infer_batch_images(model, device, source_path):
    ...
    print("💾 结果保存路径：")
    print("  - 检测图片：runs/detect/predict")
    print("  - 标注文件：runs/detect/predict/labels")
